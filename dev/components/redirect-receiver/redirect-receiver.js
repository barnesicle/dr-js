...
var REDIRECT_STORAGE_SESSION_AUTHENTICATION_ID = 'DRRedirectSessionAuthenticationId';
  ...
  return function (event) {
    ...
    if (event.key === REDIRECT_STORAGE_ACTION_KEY && isRedirectAction(event.newValue)) {
      ...
      if (redirectWindowDataWasSet(components['controller'].redirectWindowData)) {
        (0,_client_dropin_window_data__WEBPACK_IMPORTED_MODULE_1__.clearRedirectData)(components['controller'].redirectWindowData);
      }
...
function createHandleSessionAuthenticationStorageEvent(components, clientEmitter, adyenEmitter) {
  return function (event) {
    if (event.key === REDIRECT_STORAGE_SESSION_AUTHENTICATION_ID) {
      var value = event.newValue.split('_');
      var sessionAuthenticationId = value[0];
      var controllerThatInitiatedRedirect = value[1];
      ...
      adyenEmitter.send('sessionAuthenticationRedirectComplete', {
        sessionAuthenticationId: sessionAuthenticationId
      });
    }
  };
}
...
function getSessionAuthenticationIdFromQueryString(queryString) {
  var pattern = new RegExp('[A-Za-z]');
  return getQueryParameter(queryString, 'sessionAuthenticationId', pattern);
}
...
function setRedirectWindowData(redirectWindowData, redirectWindow, sendCancelEvent, paymentMethodType, resolve) {
  var timer = _babel_runtime_corejs3_core_js_stable_set_interval__WEBPACK_IMPORTED_MODULE_2___default()( /*#__PURE__*/(0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee() {
    var isClosed;
    return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return redirectWindowData.window.serializedWindow.isClosed();
...
if (actionFromQueryString) {
  ...
  //window.localStorage.setItem(REDIRECT_STORAGE_ACTION_KEY, 'unknown');
...
if (sourceAuthenticationIdFromQueryString) {
  // Note Setting to unknown is required because if it is already set, setting to the same value will not fire the storage event.
  //window.localStorage.setItem(REDIRECT_STORAGE_SOURCE_AUTHENTICATION_ID, 'unknown');
...
if (sessionAuthenticationIdFromQueryString) {
  // Note Setting to unknown is required because if it is already set, setting to the same value will not fire the storage event.
  //window.localStorage.setItem(REDIRECT_STORAGE_SESSION_AUTHENTICATION_ID, 'unknown');
...
/******/ })()
;
//# sourceMappingURL=redirect-receiver\redirect-receiver.js.map