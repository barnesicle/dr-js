...
"use strict";
/*!****************************************************************************!*\
  !*** ./src/app/components/controller/controller-giftcard-create-source.js ***!
  \****************************************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   handleGiftCardCreateSource: function() { return /* binding */ handleGiftCardCreateSource; }
/* harmony export */ });
...
function handleGiftCardCreateSource(giftCardNumberComponent, giftCardPinComponent, sourceRequest, apiKey) {
  if (!giftCardNumberComponent || !giftCardPinComponent) {
    return _babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_1___default().reject('Cannot create source because all the required components have not been created for a gift card submission');
  }
  if (!giftCardNumberComponent.send || !giftCardPinComponent.send) {
    return _babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_1___default().reject('Cannot create source because all the required components have not been mounted for a gift card submission');
  }
  var giftCardNumberPromise = createDataRetrievalPromise(giftCardNumberComponent);
  var giftCardPinPromise = createDataRetrievalPromise(giftCardPinComponent);
  var giftCardPromises = [giftCardNumberPromise, giftCardPinPromise];
  return _babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_1___default().all(giftCardPromises).then(function (values) {
    var _values = (0,_babel_runtime_corejs3_helpers_slicedToArray__WEBPACK_IMPORTED_MODULE_0__["default"])(values, 2),
      giftCardNumberElement = _values[0],
      giftCardPinElement = _values[1];
    var giftCardNumber = giftCardNumberElement.data;
    var giftCardPin = giftCardPinElement.data;
    // trigger event so that change events with errors get sent

    var triggerGiftCardEvent = giftCardNumberComponent.send('componentTriggerEvent', {
      type: 'createSource'
    });
    return triggerGiftCardEvent.then(function (triggerValues) {
      var errors = (0,_controller_create_source_utils__WEBPACK_IMPORTED_MODULE_2__.formatComponentTriggerErrors)([triggerValues]);
      if (errors.length > 0) {
        return (0,_controller_create_source_utils__WEBPACK_IMPORTED_MODULE_2__.handleCreateSourceValidation)(errors);
      }
      sourceRequest.giftCard = {
...
/******/ })()
;
//# sourceMappingURL=controller\controller-giftcard-create-source.js.map