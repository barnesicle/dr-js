var DigitalRiverBigCommerceCheckout;
...
/***/ "./src/client/BCCheckout/bc-checkout-utils.js":
/*!****************************************************!*\
  !*** ./src/client/BCCheckout/bc-checkout-utils.js ***!
  \****************************************************/
...
/* harmony export */   "addBCPriceElementObserver": function() { return /* binding */ addBCPriceElementObserver; },
/* harmony export */   "listenForNewPricesInBC": function() { return /* binding */ listenForNewPricesInBC; },
/* harmony export */   "getPrebuiltCheckoutStyle": function() { return /* binding */ getPrebuiltCheckoutStyle; },
/* harmony export */   "addPrebuiltCheckoutScript": function() { return /* binding */ addPrebuiltCheckoutScript; },
/* harmony export */   "getDRCheckout": function() { return /* binding */ getDRCheckout; },
/* harmony export */   "handleCurrencyChange": function() { return /* binding */ handleCurrencyChange; }
/* harmony export */ });
...
/* harmony import */ var _dr_api_utils__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./dr-api-utils */ "./src/client/BCCheckout/dr-api-utils.js");
/* harmony import */ var _bigCommerce_utils__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./bigCommerce-utils */ "./src/client/BCCheckout/bigCommerce-utils.js");
...
function getCookie(cookieName) {
  ...
  for (var i = 0; i < ca.length; i++) {
    ...
    if (_babel_runtime_corejs3_core_js_stable_instance_index_of__WEBPACK_IMPORTED_MODULE_3___default()(c).call(c, nameEQ) == 0) return c.substring(nameEQ.length, c.length);
  }

  return null;
} // PRICE OBSERVER FUNCTIONS

/**
 * set up observer for body changes
 */

function addBCPriceElementObserver() {
  var drBCPriceObserver = new MutationObserver(bcListen);
  drBCPriceObserver.observe(document.body, {
    subtree: true,
    childList: true,
    oldValue: true
  });
...
function bcListen(mutations) {
  listenForNewPricesInBC(mutations);
}
/**
 * determine if new price elements have been added and if so, handle them
 * @param {array} mutations
 */


function listenForNewPricesInBC(mutations) {
  _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(mutations).call(mutations, function (mutation) {
    if (mutation.target.getAttribute('data-dr-formatted-price') !== null && mutation.target.innerHTML !== mutation.target.getAttribute('data-dr-formatted-price')) {
      var _context;

      // eslint-disable-next-line no-console
      var drOriginalPrice = +_babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context = mutation.target.textContent).call(_context).replace(/^\d*\s[×x]\s/gi, '').replace(/^\D*|\D*$|,/gi, ''); // we do the set timeout to give BC a chance to update the price

      _babel_runtime_corejs3_core_js_stable_set_timeout__WEBPACK_IMPORTED_MODULE_6___default()(function () {
        mutation.target.setAttribute('data-dr-original-price', drOriginalPrice);
      }, '5');
    }
  });
}
/**
 * given an rgb value and an offset for tone, returns a darker or lighter version of the original color
 * @param {string} rgbValue - example: rgb(255,255,255)
 * @param {number} valueOffset - positive numbers for lighter tones, negative numbers for darker tones
 * @returns {string} rgb value
 */

function getTone(rgbValue, valueOffset) {
  var _context2, _context3, _context4;

  var _rgbValue$substring$s = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_7___default()(_context2 = rgbValue.substring(4, rgbValue.length - 1).split(',')).call(_context2, function (a) {
    return +a;
  }),
      _rgbValue$substring$s2 = (0,_babel_runtime_corejs3_helpers_slicedToArray__WEBPACK_IMPORTED_MODULE_1__.default)(_rgbValue$substring$s, 3),
      r = _rgbValue$substring$s2[0],
      g = _rgbValue$substring$s2[1],
      b = _rgbValue$substring$s2[2];

  return _babel_runtime_corejs3_core_js_stable_instance_concat__WEBPACK_IMPORTED_MODULE_8___default()(_context3 = _babel_runtime_corejs3_core_js_stable_instance_concat__WEBPACK_IMPORTED_MODULE_8___default()(_context4 = "rgb(".concat(r + valueOffset, ", ")).call(_context4, g + valueOffset, ", ")).call(_context3, b + valueOffset, ")");
}
/**
 * Returns the computed value for any css property of a selector
 * @param selector
 * @param property
...
function getComputedCSSValue(selector, property) {
  var el = document.querySelector(selector);
  return el !== null ? window.getComputedStyle(document.querySelector(selector), null).getPropertyValue(property) : undefined;
}
/**
 * gets a style object for Prebuilt Checkout
 * @returns {object} style object for PBC
 */


function getPrebuiltCheckoutStyle(configuration) {
  var logo = configuration.logo,
      customStyle = configuration.style;

  if (customStyle) {
    return customStyle;
  }

  var bodyColor = getComputedCSSValue('body', 'background-color');
  var bodyFont = getComputedCSSValue('body', 'font-family');
  var bodyFontColor = getComputedCSSValue('body', 'color');
  var bodyFontSize = getComputedCSSValue('body', 'font-size');
  var bodyFontStyle = getComputedCSSValue('body', 'font-style');
  var bodyFontVariant = getComputedCSSValue('body', 'font-variant');
  var bodyLinkColor = getComputedCSSValue('body a', 'color');
  var bodyLetterSpacing = getComputedCSSValue('body', 'letter-spacing');
  var headerColor = getComputedCSSValue('header', 'background-color') || bodyColor;
  var sectionHeading = getComputedCSSValue('h1', 'color') || bodyFontColor;
  var orderSummaryColor = getTone(headerColor, -10);
  var footerColor = getComputedCSSValue('footer', 'background-color') || bodyColor;
  var footerLinkColor = getComputedCSSValue('footer a', 'color') || bodyLinkColor;
  var style = {
    modal: {
      fontFamily: bodyFont,
      fontSize: bodyFontSize,
      fontStyle: bodyFontStyle,
      fontVariant: bodyFontVariant,
      themeColor: {
        primary: bodyLinkColor,
        highlight: getTone(bodyLinkColor, 20),
        background: {
          mainContent: bodyColor,
          header: headerColor,
          orderSummary: orderSummaryColor,
          footer: footerColor
        },
        text: {
          link: bodyLinkColor,
          textButton: bodyLinkColor,
          sectionHeading: sectionHeading,
          footerLink: footerLinkColor
        }
      }
    },
    textField: {
...
/**
 * Creates DigitalRiverCheckout.js script tag and appends to head and returns script
 * @returns {HTMLScriptElement}
 */

function addPrebuiltCheckoutScript() {
  var scriptDR = document.createElement('script');
  scriptDR.id = 'DigitalRiverCheckout';
  scriptDR.src = 'https://checkout.digitalriverws.com/bc/DigitalRiverCheckout.js';
  document.head.appendChild(scriptDR);
  return scriptDR;
}
/**
 *
 * @param {string} apiKey
 * @param {object} bcCurrencySettings
 * @param {object} configuration
 * @returns DigitalRiverCheckout
 */

function getDRCheckout(apiKey, bcCurrencySettings, configuration) {
  if (configuration.useDynamicPricing === true) {
    var config = {
      countrySelector: {
        containerId: configuration.currencySelectorElement || 'DR-currencySelector',
        priceElement: ['[data-product-price-with-tax]', '[data-product-non-sale-price-with-tax]', '.previewCart .productView-price', '.previewCartItem-price span', '.previewCartCheckout-price', '.navList-action', 'span.cart-item-value', '.shipping-estimate-value', '.estimator-form-input--price>label>b', '.cart-total-value>span', '.cart-item-value'],
        autoOpen: false,
        onCountrySelectorReady: function onCountrySelectorReady() {
          var saveBtn = document.getElementById('DR-currency-selector-save');
          saveBtn.addEventListener('click', function () {
            return handleCurrencyChange(apiKey, bcCurrencySettings);
          });
          saveBtn.click();
        }
      }
    };
    return new DigitalRiverCheckout(apiKey, config); // eslint-disable-line no-undef
  } else {
    return new DigitalRiverCheckout(apiKey); // eslint-disable-line no-undef
  }
}
/**
 *
 * @param {string} apiKey
 * @param {array} bcCurrencySettings
 */

function handleCurrencyChange(apiKey, bcCurrencySettings) {
  _babel_runtime_corejs3_core_js_stable_set_timeout__WEBPACK_IMPORTED_MODULE_6___default()( /*#__PURE__*/(0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_2___default().mark(function _callee() {
    var currency, country, pricingConfig, operatedByDr, baseCurrency, supportsFixedPrices, bcCart, cartHasItems, bcCurrencyConfig, bcBaseCurrencyConfig;
    return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_2___default().wrap(function _callee$(_context5) {
      while (1) {
        switch (_context5.prev = _context5.next) {
          case 0:
            currency = getCookie('dr-currency');
            country = getCookie('dr-country');
            _context5.next = 4;
            return (0,_dr_api_utils__WEBPACK_IMPORTED_MODULE_10__.getPricingConfig)(country, currency, apiKey);

          case 4:
            pricingConfig = _context5.sent;
            operatedByDr = pricingConfig.operatedByDr, baseCurrency = pricingConfig.baseCurrency, supportsFixedPrices = pricingConfig.supportsFixedPrices;
            _context5.next = 8;
            return (0,_bigCommerce_utils__WEBPACK_IMPORTED_MODULE_11__.getBigCommerceCartToken)();

          case 8:
            bcCart = _context5.sent;
            cartHasItems = bcCart.length > 0;
            bcCurrencyConfig = _babel_runtime_corejs3_core_js_stable_instance_find__WEBPACK_IMPORTED_MODULE_9___default()(bcCurrencySettings).call(bcCurrencySettings, function (a) {
              return a.currencyCode === currency;
            });
            bcBaseCurrencyConfig = _babel_runtime_corejs3_core_js_stable_instance_find__WEBPACK_IMPORTED_MODULE_9___default()(bcCurrencySettings).call(bcCurrencySettings, function (a) {
              return a.currencyCode === baseCurrency;
            });

            if (!(operatedByDr === true && supportsFixedPrices === true && typeof bcBaseCurrencyConfig !== 'undefined')) {
              _context5.next = 16;
              break;
            }

            return _context5.abrupt("return", (0,_bigCommerce_utils__WEBPACK_IMPORTED_MODULE_11__.updateBigCommerceCartCurrency)(bcBaseCurrencyConfig, currency, cartHasItems));

          case 16:
            return _context5.abrupt("return", (0,_bigCommerce_utils__WEBPACK_IMPORTED_MODULE_11__.updateBigCommerceCartCurrency)(bcCurrencyConfig, currency, cartHasItems));
...
/***/ "./src/client/BCCheckout/bigCommerce-utils.js":
/*!****************************************************!*\
  !*** ./src/client/BCCheckout/bigCommerce-utils.js ***!
  \****************************************************/
/***/ (function(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "getBigCommerceCartToken": function() { return /* binding */ getBigCommerceCartToken; },
/* harmony export */   "updateBigCommerceCartCurrency": function() { return /* binding */ updateBigCommerceCartCurrency; },
/* harmony export */   "getBigCommerceCurrencySettings": function() { return /* binding */ getBigCommerceCurrencySettings; }
/* harmony export */ });
/* harmony import */ var _babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @babel/runtime-corejs3/helpers/asyncToGenerator */ "./node_modules/@babel/runtime-corejs3/helpers/esm/asyncToGenerator.js");
 ...
 * @returns {Promise<any>}
 */
function getBigCommerceCartToken() {
...
/**
 * Updates BC cart currency
 * @param bcCurrencyConfig
 * @param currency
 * @param cartHasItems
 * @returns {Promise<void>}
 */

function updateBigCommerceCartCurrency(_x, _x2, _x3) {
  return _updateBigCommerceCartCurrency.apply(this, arguments);
}
/**
 * Gets BC's currency settings for dynamic pricing from BC's currency selector elements
 * @returns {{cartURL: {string}, preCartURL: {string}, currencyCode: {string}}[]}
 */

function _updateBigCommerceCartCurrency() {
  _updateBigCommerceCartCurrency = (0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee(bcCurrencyConfig, currency, cartHasItems) {
    var cartURL, preCartURL, requestOptions;
    return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            cartURL = bcCurrencyConfig.cartURL, preCartURL = bcCurrencyConfig.preCartURL;
            ...
            requestOptions = {
              ...
              body: _babel_runtime_corejs3_core_js_stable_json_stringify__WEBPACK_IMPORTED_MODULE_4___default()({
                currencyCode: currency
              })
            };
            return _context2.abrupt("return", fetch(cartURL, requestOptions));

          case 6:
            return _context2.abrupt("return", fetch(preCartURL));
...
function getBigCommerceCurrencySettings() {
  var _context;

  return _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_2___default()(_context = _babel_runtime_corejs3_core_js_stable_array_from__WEBPACK_IMPORTED_MODULE_3___default()(document.querySelectorAll('#currencySelection a'))).call(_context, function (a) {
    return {
      currencyCode: a.getAttribute('data-currency-code'),
      cartURL: a.getAttribute('data-cart-currency-switch-url'),
      preCartURL: a.href
    };
  });
}
...
/***/ "./src/client/BCCheckout/dr-api-utils.js":
/*!***********************************************!*\
  !*** ./src/client/BCCheckout/dr-api-utils.js ***!
  \***********************************************/
/***/ (function(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "getPricingConfig": function() { return /* binding */ getPricingConfig; }
/* harmony export */ });
...
/**
 * Get's client's DR currency settings
 * @param {string} country
 * @param {string} currency
 * @param {string} apiKey
 * @returns {Promise<any>}
 */
function getPricingConfig(country, currency, apiKey) {
  var _context;

  return fetch(_babel_runtime_corejs3_core_js_stable_instance_concat__WEBPACK_IMPORTED_MODULE_0___default()(_context = "https://api.digitalriver.com/dynamic-pricing/country-currency/countries/".concat(country, "/currencies/")).call(_context, currency), {
    headers: {
...
"use strict";
/*!*********************************************!*\
  !*** ./src/client/BCCheckout/BCCheckout.js ***!
  \*********************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": function() { return /* binding */ DigitalRiverBigCommerceCheckout; }
/* harmony export */ });
/* harmony import */ var _babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @babel/runtime-corejs3/helpers/asyncToGenerator */ "./node_modules/@babel/runtime-corejs3/helpers/esm/asyncToGenerator.js");
/* harmony import */ var _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @babel/runtime-corejs3/regenerator */ "./node_modules/@babel/runtime-corejs3/regenerator/index.js");
/* harmony import */ var _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1__);
...
/* harmony import */ var _bc_checkout_utils__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./bc-checkout-utils */ "./src/client/BCCheckout/bc-checkout-utils.js");
/* harmony import */ var _bigCommerce_utils__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./bigCommerce-utils */ "./src/client/BCCheckout/bigCommerce-utils.js");
...
(function () {
  ...
  if (!script.getAttribute('data-dr-apiKey')) {
    throw new Error('Please verify that your DR-Big-Commerce-Checkout script element contains a data-dr-apiKey attribute containing your public API key as the value');
  }

  var tyPage = script.getAttribute('data-dr-thank-you-page');
  var configuration = {
    apiKey: script.getAttribute('data-dr-apiKey'),
    currencySelectorElement: script.getAttribute('data-dr-currency-selector'),
    language: script.getAttribute('data-dr-language'),
    logo: script.getAttribute('data-dr-logo'),
    useDynamicPricing: script.getAttribute('data-dr-dynamic-pricing') === 'true',
    displayMode: script.getAttribute('data-dr-display-mode'),
    style: JSON.parse(decodeURI(script.getAttribute('data-dr-checkout-style'))),
    thankYouPage: tyPage !== null ? decodeURI(tyPage) : null,
    shipToCountries: script.getAttribute('data-dr-ship-to-countries'),
    billToCountries: script.getAttribute('data-dr-bill-to-countries'),
    customOnReady: script.getAttribute('data-dr-on-ready'),
    customOnAddressComplete: script.getAttribute('data-dr-on-address-complete'),
    customOnError: script.getAttribute('data-dr-on-error'),
    customOnClose: script.getAttribute('data-dr-on-close'),
    customOnDeliveryComplete: script.getAttribute('data-dr-on-delivery-complete'),
    customOnCheckoutComplete: script.getAttribute('data-dr-on-checkout-complete')
  };
  window.digitalRiverBigCommerceCheckout = new DigitalRiverBigCommerceCheckout(configuration);
})();

function DigitalRiverBigCommerceCheckout(configuration) {
  var dropinCheckout;
  /**
   * creates DR's prebuilt checkout
   * @param bigCommerceCheckoutId
   * @returns {Promise<*>}
   */

  function createPrebuiltCheckout(_x) {
    return _createPrebuiltCheckout.apply(this, arguments);
  }
  /**
   * sets some styles when DR's prebuilt checkout has loaded
   */


  function _createPrebuiltCheckout() {
    _createPrebuiltCheckout = (0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee(bigCommerceCheckoutId) {
      var _navigator;

      var style, options, _context, _context2, config, _yield$dropinCheckout, id;

      return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              style = (0,_bc_checkout_utils__WEBPACK_IMPORTED_MODULE_5__.getPrebuiltCheckoutStyle)(configuration);
              options = {
                style: style,
                displayMode: configuration.displayMode !== null ? configuration.displayMode : 'fullPage',
                language: configuration.language !== null ? configuration.language : (_navigator = navigator) === null || _navigator === void 0 ? void 0 : _navigator.language.split('-')[0]
              };

              if (configuration.thankYouPage !== null) {
                options.thankYouPage = configuration.thankYouPage;
              }

              if (configuration.shipToCountries !== null) {
                options.shipToCountries = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_2___default()(_context = configuration.shipToCountries.split(',')).call(_context, function (a) {
                  return _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_3___default()(a).call(a);
                });
              }

              if (configuration.billToCountries !== null) {
                options.billToCountries = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_2___default()(_context2 = configuration.billToCountries.split(',')).call(_context2, function (a) {
                  return _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_3___default()(a).call(a);
                });
              }
              ...
              _context3.next = 8;
              return dropinCheckout.createCheckoutSessionWithUpstreamToken(bigCommerceCheckoutId);

            case 8:
              _yield$dropinCheckout = _context3.sent;
              id = _yield$dropinCheckout.id;
              dropinCheckout.createModal(id, config);
              return _context3.abrupt("return", dropinCheckout);
  ...
  function handleReady() {
    document.querySelector('[id^="DRCheckoutIframe"]').style.backgroundColor = 'rgba(0,0,0,.75)';

    if (configuration.customOnReady !== null) {
      window[configuration.customOnReady]();
    }
  }
  /**
  ...
  function handleAddressComplete(address) {
    if (configuration.customOnAddressComplete !== null) {
      window[configuration.customOnAddressComplete](address);
    }
  }
  /**
   *
  ...
  function handleCheckoutComplete(order) {
    if (configuration.customOnCheckoutComplete !== null) {
      window[configuration.customOnCheckoutComplete](order);
    }
  }
  /**
   *
  ...
  function handleDeliveryComplete(shippingMethod) {
    if (configuration.customOnDeliveryComplete !== null) {
      window[configuration.customOnDeliveryComplete](shippingMethod);
    }
  }
  /**
   * reloads the page when user closes DR's prebuilt checkout
   */


  function handleCheckoutClose() {
    if (configuration.customOnClose !== null) {
      window[configuration.customOnClose]();
    } else {
  ...
  function handleError(error) {
    if (configuration.customOnError !== null) {
      window[configuration.customOnError](error);
    } else {
      throw new Error(error);
    }
  }
  /**
   * When user clicks on a checkout button, starts up DR's prebuilt checkout
   * @param e
   * @returns {Promise<*>}
   */


  function handleCheckout(_x2) {
    return _handleCheckout.apply(this, arguments);
  }
  /**
   * Replaces the BC currency selector element with our own for dynamic pricing
   */


  function _handleCheckout() {
    _handleCheckout = (0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee3(e) {
      var _context4;

      return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee3$(_context6) {
        while (1) {
          switch (_context6.prev = _context6.next) {
            case 0:
              if (!(e.target.href && typeof e.target.href === 'string' && _babel_runtime_corejs3_core_js_stable_instance_includes__WEBPACK_IMPORTED_MODULE_4___default()(_context4 = e.target.href).call(_context4, '/checkout'))) {
                _context6.next = 3;
                break;
              }

              e.preventDefault();
              return _context6.abrupt("return", (0,_bigCommerce_utils__WEBPACK_IMPORTED_MODULE_6__.getBigCommerceCartToken)().then( /*#__PURE__*/function () {
                var _ref = (0,_babel_runtime_corejs3_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__.default)( /*#__PURE__*/_babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee2(response) {
                  var bigCommerceCheckoutId;
                  return _babel_runtime_corejs3_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee2$(_context5) {
                    while (1) {
                      switch (_context5.prev = _context5.next) {
                        case 0:
                          bigCommerceCheckoutId = response[0].id;
                          _context5.next = 3;
                          return createPrebuiltCheckout(bigCommerceCheckoutId);
  ...
  function replaceCurrencySelectorEl() {
    if (configuration.useDynamicPricing === true) {
      var currencySelectorEl = document.getElementById(configuration.currencySelectorElement) || document.querySelector('[data-dropdown="currencySelection"]').parentElement;

      if (configuration.currencySelectorElement === null) {
        currencySelectorEl.id = 'DR-currencySelector';
        currencySelectorEl.style.margin = '1em';
      }
  ...
  /**
   * sets BC up to use DR Prebuilt Checkout
   */


  function setupCheckout() {
    var checkoutScript = (0,_bc_checkout_utils__WEBPACK_IMPORTED_MODULE_5__.addPrebuiltCheckoutScript)();
    checkoutScript.addEventListener('load', function () {
      var apiKey = configuration.apiKey;
      var bcCurrencySettings = (0,_bigCommerce_utils__WEBPACK_IMPORTED_MODULE_6__.getBigCommerceCurrencySettings)();
      replaceCurrencySelectorEl();
      dropinCheckout = (0,_bc_checkout_utils__WEBPACK_IMPORTED_MODULE_5__.getDRCheckout)(apiKey, bcCurrencySettings, configuration);
      document.body.addEventListener('click', handleCheckout);
      (0,_bc_checkout_utils__WEBPACK_IMPORTED_MODULE_5__.addBCPriceElementObserver)();
    });
  }
...
DigitalRiverBigCommerceCheckout = __webpack_exports__;
/******/ })()
;
//# sourceMappingURL=DigitalRiverBigCommerceCheckout.js.map